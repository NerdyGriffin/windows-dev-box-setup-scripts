#--- Game Launchers ---
winget install --id=Cemu.Cemu --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=DolphinEmulator.Dolphin --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=ElectronicArts.EADesktop --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=EpicGames.EpicGamesLauncher --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=GOG.Galaxy --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=Mojang.MinecraftLauncher --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=Mojang.MinecraftLauncher --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=Ubisoft.Connect --exact --silent --accept-package-agreements --accept-source-agreements
winget install --id=Valve.Steam --exact --silent --accept-package-agreements --accept-source-agreements
