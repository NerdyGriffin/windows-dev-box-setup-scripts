...
if (Test-Path $WindowsTerminalSettingsDir) {
	...
	if (-not(Test-Path (Join-Path $BackupDirPath 'settings.json'))) {
		Copy-Item -Path "$WindowsTerminalSettingsDir\*" -Destination $BackupDirPath -Force -Exclude $BackupDirName -Recurse
	}
}
